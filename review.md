# Review das Alterações - 25/09/2026
...
### 41. Autocriação de Lojas Personalizadas ao Conectar Contas, Integridade de Status de Instalação/Grayscale em Jogos Desinstalados e Telemetria TorBox / Auto-Update Direct-Link
* **Problema:** Ao conectar contas oficiais (Xbox, Battle.net, Ubisoft, EA Games) o Ghost não criava as lojas correspondentes nem atribuía os jogos automaticamente; títulos desinstalados da Battle.net e ao editar capas ficavam coloridos indevidamente; updates via TorBox omitiam progresso e porcentagem no card ativo de downloads; e updates de sideload falhavam com erro de opção de download indisponível.
* **Solução:**
  - **Criação Automática de Lojas Personalizadas e Atribuição Instantânea de Jogos (Conquista 112/Regra 80)**: Mapeados os 4 provedores oficiais mantendo os nomes e ícones oficiais Base64 (Xbox `store-1782835258378`, EA Games `store-1782833766006`, Battle.net `store-1790309300464`, Ubisoft `store-1790309448685`). Helper `ensureConnectedAccountCustomStore(provider)` garante criação imediata (0ms) no sucesso da conexão ou no carregamento da tela. Atribuição automática em `heroic_game_assignments` e renderização de logotipos oficiais no `StoreLogos`.
  - **Integridade de Status de Instalação e Grayscale Estrito em Jogos Desinstalados (Conquista 111/Regra 79)**: Preservação fiel de `is_installed` em `addNewApp` exigindo executável físico em disco (`existsSync`). Removido hardcode `is_installed: true` de `InlineGameSettings`. Cards desinstalados recebem `:not(.installed)` com `filter: grayscale(100%)` em repouso e ganham cor no `:hover`. Dimensões (aspect-ratio 173/275) e transparência 100% preservadas.
  - **Redirecionamento Automático para Downloads no Auto-Update, Telemetria TorBox e Preferência Inteligente de Transporte (Conquista 110/Regra 78)**: Roteamento imediato para `#/download-manager` em todos os pontos de gatilho de update. Preservada a fonte original do usuário (`anker-direct`). Telemetria em tempo real no `ExternalActiveCard` durante a fase remota TorBox com porcentagem ciano neon, banner descritivo e link alternativo para troca de fonte.
  - **Auto-Update Universal para Jogos Sideload/Externos da Loja Piratas com Proteção GhostShield (Conquista 109/Regra 77)**: Helper universal `updateGame` em `library.ts` dispara `window.api.externalGamesInstall` repassando `replaceInstallationId`. Preservação automática de saves via GhostShield.
  - **Qualidade & Testes**: `pnpm run codecheck` com 0 erros TypeScript (`tsc --noEmit`) e 184/184 testes unitários Jest aprovados.
